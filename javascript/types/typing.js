"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var name = "Henrique";
var age = 25;
var isStudent = false;
function greet(name) {
    var msg = "Hello, ".concat(name, "!");
    console.log(msg);
    return msg;
}
function proclaim(status, repeat) {
    if (repeat === void 0) { repeat = 0; }
    var actualStatus = status !== null && status !== void 0 ? status : "Success";
    for (var i = 0; i < repeat + 1; i++) {
        console.log(actualStatus);
    }
}
var hobbies = ["eat", "read", "code"];
;
function logUser(user) {
    for (var _i = 0, _a = Object.entries(user); _i < _a.length; _i++) {
        var _b = _a[_i], k = _b[0], v = _b[1];
        console.log("".concat(k, ": ").concat(v));
    }
}
var coord = [1, 2];
console.log(coord);
function formatValue(val) {
    if (typeof val === 'string') {
        console.log("".concat(val, " is a string."));
        return;
    }
    console.log("".concat(val, " is a number."));
}
var Status;
(function (Status) {
    Status[Status["SUCCESS"] = 1] = "SUCCESS";
    Status[Status["PENDING"] = 2] = "PENDING";
    Status[Status["FAILED"] = 3] = "FAILED";
})(Status || (Status = {}));
;
var status = Status.PENDING;
function identity(thing) {
    console.log("The identity of the thing is: ".concat(typeof thing));
    return thing;
}
var StorageContainer = /** @class */ (function () {
    function StorageContainer() {
        this.itemList = [];
    }
    StorageContainer.prototype.addItem = function (item) {
        this.itemList.push(item);
    };
    StorageContainer.prototype.getItem = function (index) {
        return this.itemList[index];
    };
    StorageContainer.prototype.deleteItem = function (index) {
        this.itemList.splice(index, 1);
    };
    return StorageContainer;
}());
function logUserPartial(user) {
    for (var _i = 0, _a = Object.entries(user); _i < _a.length; _i++) {
        var _b = _a[_i], k = _b[0], v = _b[1];
        console.log("".concat(k, ": ").concat(v));
    }
}
function fetchData(url) {
    return __awaiter(this, void 0, void 0, function () {
        var response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch(url)];
                case 1:
                    response = _a.sent();
                    return [4 /*yield*/, response.json()];
                case 2:
                    data = _a.sent();
                    return [2 /*return*/, data];
            }
        });
    });
}
function checkUserStatus(user) {
    return __awaiter(this, void 0, void 0, function () {
        var response, data;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetch('')];
                case 1:
                    response = _a.sent();
                    return [4 /*yield*/, response.json()];
                case 2:
                    data = _a.sent();
                    return [2 /*return*/, data];
            }
        });
    });
}
var finalizedObj = {
    id: 1,
    username: 'h',
    email: 'h@gmail.com',
    childAccounts: [
        {
            id: 2,
            username: 'ab',
        }
    ]
};
